...
void CScrollMgr::Scroll_Lock()
{
	...
	//흔들리는 중이라면
	if (GetTickCount() < m_dwShakeEndTime)
	{
		m_fShakeOffsetX = ((rand() % 200) / 100.f - 1.f) * m_fShakeIntensity; //랜덤값 부여
		m_fShakeOffsetY = ((rand() % 200) / 100.f - 1.f) * m_fShakeIntensity;
	}
	else
	{
		m_fShakeOffsetX = 0.f; //시간 지나면 다시 0으로(흔들림 종료)
		m_fShakeOffsetY = 0.f;
	}
}
...
void CScrollMgr::Start_Shake(float _fIntensity, DWORD _dwDuration)
{
	m_fShakeIntensity = _fIntensity;
	m_dwShakeEndTime = GetTickCount() + _dwDuration;
}