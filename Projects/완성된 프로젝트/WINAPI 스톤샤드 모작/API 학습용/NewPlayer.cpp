...
void CNewPlayer::Key_Check()
{
		...
		CKeyMgr::Get_Instance()->Key_Down(VK_LBUTTON))  //인벤도 vk_lbutton을 사용해서 인벤토리가 닫혀있을떄만 작동되게
	{
		...
		if (pMonster)
		{
			...
			if (iDistance <= PLAYER_ATTACK_RANGE)
			{
				...
				CSoundMgr::Get_Instance()->PlaySound(_T("snd_skill_mighty_swing.wav"), CSoundMgr::PLAYER);
				CTurnMgr::Get_Instance()->End_PlayerTurn();
			}
		}
	}
	...
	{
		//문타일 떠날때 문 닫는 사운드 재생
		int iPrevDrawID = CTileMgr::Get_Instance()->Get_TileDrawId(m_tGridPos.row, m_tGridPos.col);
		if (iPrevDrawID == 2 || iPrevDrawID == 3)  //문 타일에서 떠날 때
			CSoundMgr::Get_Instance()->PlaySound(_T("snd_wooden_door_close.wav"), CSoundMgr::PLAYER);

		Set_GridPos(row, col);

		//문타일 도착할때 문 여는 사운드 재생
		int iNewDrawID = CTileMgr::Get_Instance()->Get_TileDrawId(row, col);
		if (iNewDrawID == 2 || iNewDrawID == 3)  //문 타일에 도착할 때
			CSoundMgr::Get_Instance()->PlaySound(_T("snd_wooden_door_open.wav"), CSoundMgr::PLAYER);
		m_bKeyLock = true;
...
void CNewPlayer::Take_Damage(int _dmg)
{
	CObj::Take_Damage(_dmg);

	if (m_bDead)
	{
		CSoundMgr::Get_Instance()->PlaySound(_T("snd_human_death_2.wav"), CSoundMgr::PLAYER);
	}
}
