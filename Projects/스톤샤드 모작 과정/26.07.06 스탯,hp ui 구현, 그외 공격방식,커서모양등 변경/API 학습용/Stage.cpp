...
void CStage::Render(HDC _DC)
{
	...
	/*const list<CObj*>& monsterList = CObjMgr::Get_Instance()->Get_MonsterList();
	if (!monsterList.empty())
	{
		CObj* pMonster = monsterList.front();
		TCHAR sz2[64];
		wsprintf(sz2, _T("Monster row:%d col:%d"),
			pMonster->Get_GridPos().row,
			pMonster->Get_GridPos().col);
		TextOut(_DC, 10, 120, sz2, lstrlen(sz2));
	}
	TCHAR sz[64];
	wsprintf(sz, _T("Click row:%d col:%d"), g_iClickRow, g_iClickCol);
	SetBkMode(_DC, TRANSPARENT);
	SetTextColor(_DC, RGB(255, 255, 0));
	TextOut(_DC, 10, 100, sz, lstrlen(sz));
	*/
