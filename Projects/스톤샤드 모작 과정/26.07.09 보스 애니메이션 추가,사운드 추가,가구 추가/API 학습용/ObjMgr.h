...
public:
	...
	//변경 후
	//플레이어가 죽었을떄nullptr을 반환하게 수정함
	CObj* Get_Player() { return m_listObj[OBJID::PLAYER].empty() ? nullptr : m_listObj[OBJID::PLAYER].front(); } //m_listObj[OBJID::PLAYER].empty()가 true면 nullptr을 아니면 .front()를 반환
	CObj* Get_Target(OBJID::ID _eID, CObj* _pObj);
	CObj* Get_MonsterAt(int _row, int _col);
	...
	bool Is_EffectEmpty() { return m_listObj[OBJID::EFFECT].empty(); }
