...
bool CObjMgr::Is_Occupied(const GRIDPOS& _pos)
{
    ...
    for (auto& pProp : m_listObj[OBJID::PROP])
    {
        if (pProp->Get_GridPos() == _pos)
            return true;
    }
...
CObj* CObjMgr::Get_MonsterAt(int _row, int _col)
{
    for (auto& pObj : m_listObj[OBJID::MONSTER])
    {
        if (pObj->Get_GridPos().row == _row &&
            pObj->Get_GridPos().col == _col)
            return pObj;
    }
    return nullptr;
}