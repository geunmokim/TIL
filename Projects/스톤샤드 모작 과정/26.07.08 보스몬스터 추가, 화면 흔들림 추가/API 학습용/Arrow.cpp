...
int CArrow::Update()
{
	...
	if (m_iLifeFrame % 5 == 0)
	{
		int iNextRow = m_tGridPos.row;
		int iNextCol = m_tGridPos.col;

		if (m_tGridPos.row < m_tTargetPos.row) ++iNextRow;
		else if (m_tGridPos.row > m_tTargetPos.row) --iNextRow;

		if (m_tGridPos.col < m_tTargetPos.col) ++iNextCol;
		else if (m_tGridPos.col > m_tTargetPos.col) --iNextCol;

		Set_GridPos(iNextRow, iNextCol);

		// 타겟 도달
		if (m_tGridPos.row == m_tTargetPos.row && m_tGridPos.col == m_tTargetPos.col)
		{
			m_bDead = true;
			return OBJ_DEAD;
		}
