...
{
	//day3 추가 //imgui가 이 메시지를 먼저 보게 해줘야함
...
//nCmdShow - 창을 처음에 어떻게 보여줄지 힌트값
{
	...
	//day3 추가
	CMainGame::Get_Instance()->Init_ImGui(hWnd);
	...
	while (msg.message != WM_QUIT)
	{
		...
		CMainGame::Get_Instance()->Update();
		CDevice::Get_Instance()->Render_Begin(ClearColor);
		CMainGame::Get_Instance()->Render();
		CDevice::Get_Instance()->Render_End();
	
	}


	CMainGame::Get_Instance()->Release();
