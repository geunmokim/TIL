...
private:
	CObj* Create_Bullet();
	//다형성 덕분에 부모가 같다면 부모* 다른 자식의 클래스();식으로 다른 자식의 클래스도 가져와 사용할 수 있다
	//// 반환 타입이 CObj* 이지만
    // 실제로는 CBullet 객체를 만들어서 반환
	/*
    CBullet이 CObj를 상속받으므로
    CBullet* → CObj* 로 변환 가능 (업캐스팅)

    CObj*    → CBullet* 는 불가 (다운캐스팅, 위험)
	*/
